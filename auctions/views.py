...
def new_listing(request):
    if request.method == "POST":
        ...
        listing.save() # This seals the deal
        return HttpResponseRedirect(reverse("index"))
    return render(request, "auctions/create.html")

def watch(request):
    # First get the object from class
    # Now update the variable for Watchlist set it True
    pass

def watchlist(request):
    # This would require all objs with tags 
    all_listings = Auction_Listing.objects.exclude(watch=True)
    if not all_listings:
